...
def calc_thermal_nox(
    ...
    """
    サーマルNOx推定（カスタム混合ガス対応）

    Parameters
    ----------
    composition   : {formula: mol_fraction}（合計1.0に正規化）
    lambda_val    : 空気過剰率 λ（空気比）
    T_K           : 初期温度 [K]
    P_Pa          : 初期圧力 [Pa]
    ...
    -------
    dict  以下のキーを含む：
        v_fuel_ms         : 燃料噴出速度 [m/s]
        tau_ms            : 推定火炎滞留時間 [ms]
        T_adiabatic_C     : 断熱火炎温度 [℃]
        NOx_equilibrium_ppm : Cantera平衡NOx（理論上限）[ppm]
        NOx_thermal_ppm   : 拡張Zeldovichモデル推定値 [ppm]
        NOx_thermal_mg_Nm3: 同上 [mg/Nm³]（O₂=0%換算）
        method            : 使用した計算手法の説明
        warning           : 注意事項（あれば）
    """
    ...
    v_fuel = Q_m3s / max(A_m2, 1e-12)          # [m/s]
    tau_s  = L_flame_m / max(v_fuel, 0.01)      # 火炎滞留時間 [s]
    tau_ms = tau_s * 1000.0
    ...
    xO_post   = 0.0
    xOH_post  = 0.0
    method    = "Zeldovich積分近似（Cantera解析フォールバック）"

    try:
        import cantera as ct
        mix_str = _build_ct_mix_string(comp_norm, lambda_val)

        if mix_str is None:
            warnings.append(
                "gri30非対応成分（C4以上・H2S・DME等）を含むため"
                "Cantera計算をスキップ。解析近似で推定します。"
            )
        else:
            gas = ct.Solution(_get_gri30_path())
            gas.TPX = T_K, P_Pa, mix_str
            ...
            NO2_eq_ppm = gas["NO2"].X[0] * 1e6
            NOx_eq_ppm = NO_eq_ppm + NO2_eq_ppm

            xO2_post  = float(gas["O2"].X[0])
            xN2_post  = float(gas["N2"].X[0])
            xH2O_post = float(gas["H2O"].X[0])
            xO_post   = float(gas["O"].X[0])
            xOH_post  = float(gas["OH"].X[0])
            method    = "Cantera平衡（gri30.yaml）+ 拡張Zeldovich積分"

    except Exception as e:
        import traceback as _tb
        warnings.append(
            f"Cantera計算エラー: {type(e).__name__}: {e}\n"
            f"  詳細: {_tb.format_exc().splitlines()[-3] if _tb.format_exc() else ''}"
        )

    # ── 断熱火炎温度（Canterra計算なしの場合は解析近似） ──
    if T_ad is None:
        T_ad_approx = _calc_mixture_Tad_analytical(
            comp_norm, lambda_val, T_K,
            o2_required_total=sum(comp_norm.get(f, 0)*o for f, o in _O2_REQ.items()),
            o2_in_fuel=comp_norm.get("O2", 0.0),
            inert_in_fuel={f: comp_norm[f] for f in ("N2","CO2","Ar","He","H2O") if f in comp_norm},
            co2_total=sum(comp_norm.get(f,0)*c for f,(_,c,*_) in _COMB.items()),
            h2o_total=sum(comp_norm.get(f,0)*h for f,(_,__,h,*_) in _COMB.items()),
            so2_total=sum(comp_norm.get(f,0)*s for f,(*_,s) in _COMB.items()),
        )
        T_ad = (T_ad_approx + 273.15) if T_ad_approx else (T_K + 1800)
    ...
    if xO_post > 0:
        # Cantera で O, OH ラジカル取得済み
        C_O  = xO_post  * C_tot
        C_OH = xOH_post * C_tot
    else:
        # 部分平衡近似（O2 ⇌ 2O）
        P_atm  = P_Pa / 101325.0
        Kp_O   = _math.exp(-59380 / T_f + 10.66) if T_f > 1000 else 1e-20
        xO2_eff = max(xO2_post, 1e-6)
        xO     = _math.sqrt(max(Kp_O * xO2_eff / P_atm, 0))
        C_O    = xO * C_tot
        Kp_OH  = _math.exp(-17400 / T_f + 9.8) if T_f > 1000 else 1e-20
        xOH    = _math.sqrt(max(Kp_OH * xH2O_post * xO2_eff / P_atm, 0))
        C_OH   = xOH * C_tot

    C_O2 = xO2_post * C_tot
    C_N2 = xN2_post * C_tot

    # Zeldovich 速度定数 [m³/mol/s]
    k1f = 1.8e8  * _math.exp(-38370 / T_f)        # N2 + O → NO + N
    k2f = 1.8e4  * T_f * _math.exp(-4680 / T_f)   # N  + O2 → NO + O
    k3f = 3.8e7  * _math.exp(-85 / T_f)           # N  + OH → NO + H

    denom  = 1.0 + k1f * C_O / max(k2f * C_O2 + k3f * C_OH, 1e-30)
    dNO_dt = 2.0 * k1f * C_O * C_N2 / denom      # [mol/m³/s]
    C_NO   = dNO_dt * tau_s                        # [mol/m³]

    NO_zel_ppm = C_NO / max(C_tot, 1e-30) * 1e6   # [ppm vol]

    # 平衡値を上限に（物理的制約）
    if NOx_eq_ppm is not None:
        NO_zel_ppm = min(NO_zel_ppm, NOx_eq_ppm)

    # [mg/Nm³] 換算（NO 分子量 30 g/mol）
    # 1 ppm(vol) = 30/22414 g/Nm³ = 1.339 mg/Nm³ （0℃, 1atm基準）
    # 1 ppm(vol) = 1e-6 mol/mol × (1000/22.414 mol/Nm³) × 30 g/mol × 1000 mg/g
    # = 30 / 22.414 mg/Nm³
    NO_MG_PER_PPM = 30.0 / 22.414          # [mg/Nm³ per ppm]
    NOx_mg_Nm3 = NO_zel_ppm * NO_MG_PER_PPM

    # 高温注意（2200K超はZeldovich感度が高く誤差大きい）
    if T_ad > 2200:
        warnings.append(
            f"断熱火炎温度 {T_ad-273.15:.0f}℃ は極めて高温です。"
            "Zeldovich積分は推定誤差が大きくなります。"
            "平衡値（理論上限）も参照してください。"
        )
    if v_fuel > 50:
        warnings.append(
            f"燃料噴出速度 {v_fuel:.1f} m/s が高速です。"
            "実際の滞留時間は火炎の安定・不安定条件に依存します。"
        )
    ...
    return {
        ...
        # ── NOx推定値 ──
        "NOx_equilibrium_ppm":  round(NOx_eq_ppm, 1) if NOx_eq_ppm is not None else None,
        "NO_equilibrium_ppm":   round(NO_eq_ppm,  1) if NO_eq_ppm  is not None else None,
        "NOx_thermal_ppm":      round(NO_zel_ppm, 1),
        "NOx_thermal_mg_Nm3":   round(NOx_mg_Nm3, 1),
        # ── 補足情報 ──
        "method":   method,
        "warnings": warnings,
    }
