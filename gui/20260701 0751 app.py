...
class OrificeCalculatorApp:
    ...
    def _save_settings(self):
        ...
        try:
            ...
            pass  # 保存失敗は無視（次回起動時にデフォルト値で開くだけ）
    ...
    def __init__(self, root):
        ...
        # 前回設定を読み込んでおく（_build_ui内で各Varへ反映）
    ...
    def _build_ui(self):
        ...
        if s:
            try:
                ...
                if "pipe_mat" in s:
                    self.pipe_mat_var.set(s["pipe_mat"])
                if "plate_mat" in s:
                    self.plate_mat_var.set(s["plate_mat"])
                if "pipe_size" in s:
                    self.pipe_size_var.set(s["pipe_size"])
                if "D" in s:
                    self.D_var.set(s["D"])
                if "d" in s:
                    self.d_var.set(s["d"])
                if "P1" in s:
                    self.P1_var.set(s["P1"])
                if "deltaP" in s:
                    self.deltaP_var.set(s["deltaP"])
                if "T" in s:
                    self.T_var.set(s["T"])
                if "z_model" in s:
                    self.z_model_var.set(s["z_model"])
    ...
    def show_combustion(self):
        ...
        _place_beside_main()
        # オリフィスGUI の Configure（移動・リサイズ）に追従
        self.root.bind("<Configure>", _follow_main)
        # 燃焼特性ウィンドウを閉じたらバインドを解除
        ...
        def recalc():
            ...
            RHO_AIR_NM3 = 1.2928  # 空気密度 kg/Nm³（0℃, 101.325 kPa）
            sg = rho_mix / RHO_AIR_NM3 if RHO_AIR_NM3 else 0.0
            MJ_TO_KCAL = 238.846  # 1 MJ = 238.846 kcal
            hhv_mj  = t['HHV_MJ_Nm3']
            lhv_mj  = t['LHV_MJ_Nm3']
            hhv_kcal = hhv_mj * MJ_TO_KCAL
            lhv_kcal = lhv_mj * MJ_TO_KCAL
            total_lbl.config(text=(
                f"  密度: {rho_mix:.5f} kg/Nm³    比重(空気=1): {sg:.4f}\n"
                f"  HHV: {hhv_mj:.4f} MJ/Nm³  ({hhv_kcal:.1f} kcal/Nm³)    "
                f"LHV: {lhv_mj:.4f} MJ/Nm³  ({lhv_kcal:.1f} kcal/Nm³)\n"
                f"{o2_line}"
