...
class OrificeCalculatorApp:
    ...
    def __init__(self, root):
        ...
        # 要素: callable(mx, my, mw) → True(生存) / False(破棄済み)
    ...
    def _create_custom_mixture(self):
        ...
        # オリフィスGUI左隣に初期配置
        self.root.update_idletasks()
        mx0 = self.root.winfo_x()
        my0 = self.root.winfo_y()
        dialog.geometry(f"340x640+{mx0 - 340 - 4}+{my0}")

        def _follower_dialog(mx, my, mw):
            """追従コールバック。ウィンドウが存在すればTrueを返す。"""
