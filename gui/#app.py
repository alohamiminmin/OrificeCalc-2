...
class OrificeCalculatorApp:
    ...
    def show_combustion(self):
        ...
        def calc_nox():
            """サーマルNOx計算（3モデル）"""
            try:
                from core.combustion import calc_thermal_nox, _calc_cantera_equilibrium
                T_K  = T_var.get() + 273.15
                P_Pa = P_var.get() * 1000.0
                lam  = max(lam_var.get(), 0.01)
                Q    = Q_var.get()
                ...
                # Cantera 平衡計算を先に実行（燃焼特性と共用 → N/A回避）
                total = sum(comp.values())
                comp_norm = {k: v/total for k, v in comp.items()} if total > 0 else comp
                eq = _calc_cantera_equilibrium(comp_norm, lam, T_K, P_Pa)
                ...
                nr = calc_thermal_nox(
                    ...
                    D_burner_mm=D,
                    L_flame_m=L,
                    cantera_eq=eq,   # 計算済み平衡値を渡す
                )
                win._last_nox = nr

                eq_str = (f"{nr['NOx_equilibrium_ppm']:.1f} ppm"
                          if nr.get("NOx_equilibrium_ppm") is not None
                          else "N/A（gri30非対応成分を含む混合ガス）")

                nox_lbl.config(foreground="black", text=(
                    f"  燃料噴出速度:       {nr['v_fuel_ms']:.3f} m/s\n"
                    f"  推定火炎滞留時間:   {nr['tau_ms']:.2f} ms\n"
                    f"  断熱火炎温度:       {nr['T_adiabatic_C']:.1f} ℃\n"
                    f"  ───────────────────────────────────────────────\n"
                    f"  【後炎冷却モデル】  {nr['NOx_cooling_ppm']:.1f} ppm"
                    f"  /  {nr['NOx_thermal_mg_Nm3']:.1f} mg/Nm³  ← 推奨\n"
                    f"  【Zeldovich推定】   {nr['NOx_zeldovich_ppm']:.1f} ppm"
                    f"  （T_ad一定・過大評価傾向）\n"
                    f"  【Cantera平衡上限】 {eq_str}"
                    f"  （理論上限・非現実的）\n"
                    f"  ───────────────────────────────────────────────\n"
                    f"  計算手法: {nr['method']}"
                ))
